...
function button()
	...
	# Registers callbacks
	IupSetCallback(btn_exit,   "ACTION", btn_exit_cb)
	IupSetCallback(btn_on_off, "ACTION", btn_on_off_cb)
	IupSetCallback(btn_image,  "ACTION", btn_image_button_cb)

	IupSetAttribute(btn_big, "BUTTON_CB", "bigtest");
	IupSetFunction("bigtest", btn_big_button_cb)
...
function btn_image_button_fn(self::Ptr{Ihandle}, b::Cint, e::Cint, x::Cint, y::Cint, t::Ptr{UInt8})
...
btn_image_button_cb = @cfunction(btn_image_button_fn, Int, (Ptr{Ihandle}, Cint, Cint, Cint, Cint, Ptr{UInt8}))

function btn_on_off_fn()
	btn_image = IupGetHandle("btn_image")		#Recovers "btn_image" handle

	# If the button with with image is active...
	if(IupGetAttribute(btn_image, "ACTIVE") == "YES")
...
btn_on_off_cb = @cfunction(btn_on_off_fn, Int, (),)

function btn_exit_fn()
  	return IUP_CLOSE		# Exits the program
end

btn_exit_cb = @cfunction(btn_exit_fn,   Int, (),)

function btn_big_button_fn(self::Ptr{Ihandle}, button::Cint, press::Cint)
	@printf("BUTTON_CB(button=%d, press=%d)\n", button, press)
	return IUP_DEFAULT
end

btn_big_button_cb = @cfunction(btn_big_button_fn, Int, (Ptr{Ihandle}, Cint, Cint))
